...
import { FaMapMarkerAlt, FaCarSide, FaUserFriends, FaRupeeSign, FaFilter } from 'react-icons/fa';
...
function SearchTrips() {
  ...
  const filteredTrips = tripsData.filter(trip => {
    ...
    const matchesPrice = !filters.price || trip.price <= parseInt(filters.price);
    const matchesPeople = !filters.people || trip.people >= parseInt(filters.people);
    ...
    <div className="page-container">
      ...
      <div className={`filters-drawer ${showFilters ? 'open' : ''}`}>
        <div className="filters-header">
          <h2>Filters</h2>
          <button className="close-btn" onClick={toggleFilters}>×</button>
        </div>
        <div className="filter-item">
          <label><FaCarSide className="input-icon" /> Car Type</label>
          <select name="carType" onChange={handleFilterChange} value={filters.carType}>
        ...
        <div className="filter-item">
          <label><FaRupeeSign className="input-icon" /> Max Price</label>
          <input type="number" name="price" placeholder="₹ Max" onChange={handleFilterChange} value={filters.price} />
        </div>
        <div className="filter-item">
          <label><FaUserFriends className="input-icon" /> People</label>
          <input type="number" name="people" placeholder="Min people" onChange={handleFilterChange} value={filters.people} />
        </div>
